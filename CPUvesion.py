...
os.environ["YOLO_VERBOSE"] = "False"     # 禁止YOLO详细输出
...
# 对于ONNX Runtime，强制设置CPU提供者
try:
    import onnxruntime as ort
    # 在全局范围设置ONNX Runtime提供者
    ort.set_default_logger_severity(3)  # 减少ONNX Runtime日志输出
    print("ONNX Runtime已设置为减少日志输出")
except ImportError:
    pass
...
def get_roi(frame, window_name="Select ROI", instruction="draw the area,then press ENTER..."):
...
def process_frame(frame, model, roi=None, alarm_cooldown=0):
    ...
    if roi:
        ...
        # 使用模型仅对ROI区域进行预测，强制使用CPU，禁止打印详细信息
        results = model(roi_image, verbose=False, device='cpu')
        alarm = False
        obj_count = 0
...
def process_image(model, image_path, window_width=1280, window_height=720, save_path=None):
    ...
    # 处理图像（使用CPU）
    result_image, alarm, _ = process_frame(image, model, roi)
...
def process_video(model, video_path, window_width=1280, window_height=720, save_path=None, conf_threshold=0.25, alert_save_dir=None):
    ...
    cv2.resizeWindow('Video Detection', window_width, window_height)
    
    frame_count = 0
    alarm_cooldown = 0  # 警报冷却时间（秒）
    alarm_interval = 3  # 警报间隔（秒）
    
    print("正在使用CPU进行推理，速度可能较慢...")
    ...
    while True:
        ...
        # 处理帧（使用CPU）
        result_frame, alarm, timestamp = process_frame(frame, model, roi, alarm_cooldown)
...
def main(config_path='config.yaml'):
    ...
    # 强制禁用CUDA和GPU加速（已在文件开头设置）
    print("已通过环境变量强制禁用所有CUDA设备")
    
    # 加载模型（强制使用CPU）
    print(f"加载模型: {model_path}")
    print("注意：此版本已完全禁用GPU加速，仅使用CPU进行推理")
    
    # 如果是ONNX模型，提供额外说明
    if model_path.endswith('.onnx'):
        print("提示：ONNX模型可能仍显示CUDA信息，但实际运行在CPU上")
        print("如需完全避免CUDA相关信息，建议使用.pt格式的PyTorch模型")
    ...
    if source.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tif', '.tiff')):
        process_image(model, source, window_width, window_height, save_path)
    else:
        # 设置保存路径
        if save_path and not save_path.lower().endswith('.mp4'):
            save_path = os.path.splitext(save_path)[0] + '.mp4'
        
        process_video(model, source, window_width, window_height, save_path, conf_threshold, alert_save_dir)
